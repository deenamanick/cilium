...
// CiliumInstallHelm...
func (kub *Kubectl) CiliumInstallHelm(version, namespace, option string, options map[string]string) *CmdRes {
	optionsString := ""
	...
	return kub.ExecMiddle(fmt.Sprintf("$HOME/local/bin/helm %s cilium cilium/cilium "+
		"--version %s "+
		"--namespace=%s "+
		"%s", option, version, namespace, optionsString))
}
