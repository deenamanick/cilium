				...
				Context("Tests with MetalLB", func() {
					...
					It("Connectivity to endpoint via LB", func() {
						lbIP, err := kubectl.GetLoadBalancerIP(
							helpers.DefaultNamespace, "test-lb-http", 30*time.Second)
						Expect(err).Should(BeNil(), "Cannot retrieve loadbalancer IP for test-lb-http")
						doRequestsFromThirdHost("http://"+lbIP, 10, false)

						lbIP, err = kubectl.GetLoadBalancerIP(
							helpers.DefaultNamespace, "test-lb-tftp", 30*time.Second)
						Expect(err).Should(BeNil(), "Cannot retrieve loadbalancer IP for test-lb-tftp")
